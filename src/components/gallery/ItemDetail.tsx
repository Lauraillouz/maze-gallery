...
function ImagePreview({ item, accent }: { item: ContentItem; accent: string }) {
  ...
  if (item.type === ContentType.Artwork) {
    return (
      <div className="flex h-full w-full items-center justify-center" style={{ background: `${accent}10` }}>
        <div style={{ width: '55%', height: '65%', background: `${accent}22`, border: `1px solid ${accent}44` }} />
      </div>
    )
  }
  if (item.type === ContentType.Book) {
    return (
      <div className="flex h-full w-full items-center justify-center" style={{ background: `${accent}10` }}>
        <div style={{ width: '45%', height: '65%', background: `${accent}22`, border: `1px solid ${accent}55`, borderRadius: 2, position: 'relative' }}>
          <div style={{ position: 'absolute', left: 0, top: 0, width: 8, height: '100%', background: `${accent}44` }} />
        </div>
      </div>
    )
  }
  ...
  // Article
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-2 px-8" style={{ background: `${accent}10` }}>
      {[90, 70, 85, 60, 80, 65, 75].map((w, i) => (
        <div key={i} style={{ height: 2, width: `${w}%`, background: `${accent}44` }} />
      ))}
    </div>
  )
}
...
interface Props {
  item: ContentItem
  onClose: () => void
  onAddToCart: (item: ContentItem) => void
}

export default function ItemDetail({ item, onClose, onAddToCart }: Props) {
  ...
  return (
    ...
    >
      <div
        ref={panelRef}
        className="relative flex w-full max-w-xl flex-col overflow-hidden sm:flex-row"
        style={{ background: '#0D0118', border: `1px solid ${accent}33`, maxHeight: '90vh' }}
      ...
      >
        ...
        <div className="flex flex-col gap-3 overflow-y-auto p-6">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              onClick={close}
              className="shrink-0 font-grotesk text-[9px] font-bold uppercase tracking-widest text-[#FFFBE0]/30 hover:text-[#FFFBE0]/70 touch-manipulation"
            >
              ✕
            </button>
          </div>
          ...
          <div className="flex flex-col gap-1">
            {item.medium && (
              <p className="font-grotesk text-[10px] text-[#FFFBE0]/40">{item.medium}</p>
            )}
            {item.size && (
              <p className="font-grotesk text-[10px] text-[#FFFBE0]/40">{item.size}</p>
            )}
            {item.editionSize && (
              <p className="font-grotesk text-[10px]" style={{ color: `${accent}88` }}>
                Edition {item.editionNumber}/{item.editionSize}
              </p>
            )}
          </div>
          ...
          {/* Description */}
          {item.description && (
            <p className="font-grotesk text-[11px] leading-relaxed text-[#FFFBE0]/60">
              {item.description}
            </p>
          )}
          ...
          <div className="mt-auto flex items-center justify-between gap-4 pt-2" style={{ borderTop: `1px solid ${accent}22` }}>
            {item.forSale && item.price !== undefined ? (
              <>
                <span className="font-grotesk text-lg font-bold" style={{ color: accent }}>
                  ${item.price.toLocaleString()}
                </span>
                <button
                  onClick={() => { onAddToCart(item); close() }}
                  className="px-4 py-2 font-grotesk text-[10px] font-bold uppercase tracking-widest text-[#08000F] touch-manipulation"
              ...
              <span className="font-grotesk text-[10px] uppercase tracking-widest text-[#FFFBE0]/25">
                {item.forSale ? 'Contact for price' : 'Not for sale'}
              </span>
            )}
          </div>
        </div>
