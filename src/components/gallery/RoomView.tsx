...
// Wider hit areas on touch: back wall almost full width, side walls full height strip
const DOOR_S_MOBILE: [number, number] = [0.1, 0.9];
const DOOR_T_MOBILE: [number, number] = [0.3, 1.0];
...
function getDoorBox(
  ...
  const vpx = w / 2 + vpOffsetX,
    vpy = h * 0.42;
  const bx0 = vpx - w * 0.16,
    bx1 = vpx + w * 0.16;
  const by0 = vpy - h * 0.25,
    by1 = vpy + h * 0.25;
  const [s0, s1] = mobile ? DOOR_S_MOBILE : DOOR_S;
  const [t0, t1] = mobile ? DOOR_T_MOBILE : DOOR_T;
...
function initialPos(
  ...
  // Cap spread so items stay on screen on narrow viewports
  const spread =
    total > 1 ? (idx / (total - 1) - 0.5) * Math.min(220, sw * 0.35) : 0;
  switch (item.wall) {
...
// Max VP shift on desktop; scales down on tablet, disabled on mobile (touch has no hover)
const VP_PULL_MAX = 250;
...
export default function RoomView({ room, onNavigate }: Props) {
  ...
  // Track horizontal swipe start for turn gesture on touch
  const swipeTouchStartRef = useRef<number | null>(null);

  // < 768px = mobile/tablet in portrait; no hover effects, wider hit areas, swipe to turn
  const isMobile = sw > 0 && sw < 768;
  // VP pull scales with screen on tablet, off on mobile (hover doesn't exist on touch)
  // On mobile: small pull for the subtle perspective lean, full pull on desktop
  const sideDoorPull = isMobile
    ? Math.min(80, sw * 0.15)
    : Math.min(VP_PULL_MAX, sw * 0.5);
  // Zoom scale that always fits within viewport height with breathing room
  const zoomScale = sh > 0 ? Math.min(2.4, (sh * 0.75) / 164) : 2.4;
  ...
  function addRipple(x: number, y: number) {
    ...
    const ALL_SURFACES: WallSurface[] = [
      "back",
      "floor",
      "ceiling",
      "left",
  ...
  // Label only — lives inside the zoom wrapper so it scales with the canvas
  function renderDoorLabel(
    side: "back" | "left" | "right",
    conn: (typeof room.connections)[0],
  ) {
    const box = getDoorBox(side, sw, sh, isMobile, vpOffX);
    const cx = box.x + box.w / 2;
    const cy = side === "back" ? box.y + box.h * 0.88 : box.y + box.h / 2;
    const rotate = side === "left" ? -90 : side === "right" ? 90 : 0;
    return (
      <span
        key={side}
        className="pointer-events-none absolute font-grotesk text-[9px] font-bold uppercase tracking-widest transition-opacity duration-200"
        style={{
          left: cx,
          top: cy,
          transform: `translate(-50%, -50%) rotate(${rotate}deg)`,
          color: ROOM_ACCENT[conn.to],
          opacity: hoveredDoor === side ? 1 : 0.4,
        }}
      >
        {ROOMS[conn.to].name}
      </span>
  ...
  // Hitbox-only button — no label, stable position outside the zoom wrapper
  function renderDoorButton(
    side: "back" | "left" | "right",
    conn: (typeof room.connections)[0],
  ) {
    const box = getDoorBox(side, sw, sh, isMobile);
    const pullX =
      side === "left" ? sideDoorPull : side === "right" ? -sideDoorPull : 0;
  ...
  // No VP shift on mobile (hover doesn't apply on touch)
  const vpOffX =
    hoveredDoor && hoveredDoor !== "back"
      ? hoveredDoor === "left"
        ? sideDoorPull
        : -sideDoorPull
      : 0;
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div
        ...
        onTouchStart={(e) => {
          // Only track swipe start if not touching a draggable content item
          if (!(e.target as HTMLElement).closest('[class*="cursor-grab"]'))
        ...
        onTouchEnd={(e) => {
          ...
          // 60px threshold to distinguish a swipe from a tap
          if (Math.abs(dx) > 60) turn(dx < 0 ? "right" : "left");
        }}
      >
        <div
          ref={zoomWrapperRef}
        ...
        >
          ...
          {sw > 0 && (
            <>
              {backConn && renderDoorLabel("back", backConn)}
              {leftConn && renderDoorLabel("left", leftConn)}
              {rightConn && renderDoorLabel("right", rightConn)}
            </>
          )}
        </div>
      ...
      {zoomedItem && (
        ...
        >
          <div
            style={{ transform: `scale(${zoomScale})`, pointerEvents: "none" }}
          >
            <ContentPlaceholder item={zoomedItem} />
          </div>
          <button
            className="absolute right-6 top-6 p-3 font-grotesk text-[10px] font-bold uppercase tracking-widest text-[#FFFBE0]/50 hover:text-[#FFFBE0] touch-manipulation"
            onClick={() => setZoomedItem(null)}
          >
            Close
          </button>
      ...
      <div className="absolute bottom-5 left-1/2 z-30 flex -translate-x-1/2 items-center gap-3">
        <button
          onClick={() => turn("left")}
          className="px-4 py-3 font-grotesk text-[10px] font-bold uppercase tracking-widest text-[#FFFBE0]/40 transition-colors hover:text-[#FFFBE0]/80 touch-manipulation"
        >
          ← Turn
        </button>
