...
function GalleryInner() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#08000F]">
      ...
      {/* Cart button */}
      {totalCount > 0 && (
        <button className="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 border border-[#FFFBE0]/20 bg-[#08000F]/90 px-4 py-2 font-grotesk text-[10px] font-bold uppercase tracking-widest text-[#FFFBE0]/60 hover:border-[#FFFBE0]/40 hover:text-[#FFFBE0]/90 touch-manipulation">
          Cart · {totalCount}
        </button>
      )}

      {/* Exit button */}
      <button ref={exitBtnRef} onClick={exitGallery} className="fixed right-4 top-4 z-50 border bg-[#08000F]/70 px-3 py-2 font-grotesk text-[10px] font-bold uppercase tracking-widest touch-manipulation" style={{ color: '#FF2D9B', borderColor: '#FF2D9B' }}>
        ⊗ leave
      </button>
    </div>
  )
